    ...
    <div className="bg-[#FDF7F2] min-h-screen">
      ...
      <header className="flex items-center justify-between px-10 py-4 bg-white shadow">
        ...
        <nav className="flex gap-6 text-sm font-medium">
          ...
          {/* Hiển thị Đơn hàng nếu đã đăng nhập */}
          {isLoggedIn && (
            <span onClick={() => navigate('/order')} className="cursor-pointer hover:text-orange-500">Đơn hàng</span>
          )}
        ...
        <div className="flex items-center gap-3">
          <input
            type="text"
            placeholder="Tìm món..."
            className="border px-4 py-2 rounded-full text-sm outline-none focus:ring-1 focus:ring-orange-500"
          />
          ...
          {!isLoggedIn ? (
            ...
            <button
              onClick={logout}
              className="text-sm px-4 py-2 bg-red-500 text-white rounded-full hover:bg-red-600"
            >
              Đăng xuất
            </button>
          )}
      ...
      <section className="relative h-[400px]">
        ...
        <button
          onClick={prevSlide}
          className="absolute left-5 top-1/2 -translate-y-1/2 bg-orange-500/80 text-white w-10 h-10 rounded-full hover:bg-orange-600"
        >
          ❮
        </button>
        <button
          onClick={nextSlide}
          className="absolute right-5 top-1/2 -translate-y-1/2 bg-orange-500/80 text-white w-10 h-10 rounded-full hover:bg-orange-600"
        >
          ❯
        </button>
      ...
      <section className="container mx-auto px-4 py-10">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {products.map((item) => (
            <div key={item.id} className="bg-white p-4 rounded-xl shadow hover:shadow-lg transition-shadow">
              <img
                src={item.img}
                alt={item.name}
